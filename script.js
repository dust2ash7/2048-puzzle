<script>
// ====================== 2048 - V4 ULTIMATE (Best Sounds + Confetti + Polish) ======================
...
function playSound(type, value = 0) {
    ...
    try {
        if (type === "move") {
            ...
            const melody = [920, 1240, 1480, 1760, 1980];
            ...
            [580, 460, 360, 260].forEach((freq, i) => {
                setTimeout(() => {
                    const o = audioContext.createOscillator();
                    const g = audioContext.createGain();
                    o.type = "sawtooth";
                    ...
                    o.connect(g).connect(audioContext.destination);
                    o.start();
                    o.stop(audioContext.currentTime + 1.4);
                }, i * 180);
            });
        }
    } catch(e) {}
...
document.getElementById("watch-ad-btn").addEventListener("click", () => {
    ...
    const int = setInterval(() => {
        t--; cd.textContent = t; pg.style.width = `${(6-t)*16.67}%`;
        if (t <= 0) {
            clearInterval(int);
